...
const AddToCartButton = ({ product, size }: ButtonProps) => {
  ...
  return itemCount > 0 ? (
    <div
      className={`flex h-full w-full justify-around rounded-lg uppercase font-bold text-sm bg-[#008081] cursor-pointer ${
    ...
    >
      ...
      <button
        onClick={(e) => add(e)}
        type="button"
        className="flex items-center justify-center w-8 bg-[#008081]"
      >
    ...
    <button
      type="button"
      className={`_add_to_cart ${size === 'lg' ? 'text-md' : 'text-sm'} bg-teal-500 text-white`}
